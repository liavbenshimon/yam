...
export default function InspectionForm({ managerId, managerName }: InspectionFormProps) {
  ...
  const [generalItems, setGeneralItems] = useState<CheckItem[]>([
    { id: "garden", topic: "מצב הגינה+ פרחים ודשא", isOk: null, isNotOk: null, comments: "" },
    { id: "mailboxNames", topic: "שמות לתיבות דואר", isOk: null, isNotOk: null, comments: "" },
    { id: "buildingFrontCleanliness", topic: "ניקיון כללי של חזית הבניין", isOk: null, isNotOk: null, comments: "" },
    {
      id: "lobbyCleanliness",
    ...
    { id: "garbageRoomCleanliness", topic: "ניקיון חדר האשפה", isOk: null, isNotOk: null, comments: "" },
    { id: "floorsCleanliness", topic: "ניקיון כללי בקומות מגורים", isOk: null, isNotOk: null, comments: "" },
    { id: "stairwellCleanliness", topic: "ניקיון חדרי המדרגות", isOk: null, isNotOk: null, comments: "" },
    {
      id: "stairwellDoors",
    ...
    { id: "stairwellItems", topic: "פינוי חפצים בחדר מדרגות", isOk: null, isNotOk: null, comments: "" },
    { id: "parkingCleanliness", topic: "ניקיון החניון תחתון", isOk: null, isNotOk: null, comments: "" },
    { id: "parkingDrainage", topic: "בדק ניקוזים בחניון", isOk: null, isNotOk: null, comments: "" },
    { id: "parkingItems", topic: "פינוי חפצים בחניון", isOk: null, isNotOk: null, comments: "" },
    { id: "clubCleanliness", topic: "ניקיון מועדון", isOk: null, isNotOk: null, comments: "" },
    { id: "roofCleanliness", topic: "בדק ניקיון הגג וניקוזים", isOk: null, isNotOk: null, comments: "" },
    { id: "stairsDoors", topic: "בדיקת הדלתות במדרגות שנסגרות", isOk: null, isNotOk: null, comments: "" },
    { id: "lighting", topic: "בדק כללי של תאורה/ החלפת נורות", isOk: null, isNotOk: null, comments: "" },
    { id: "timers", topic: "כוון שעוני שבת לפי האור", isOk: null, isNotOk: null, comments: "" },
  ])
  ...
  const [systemItems, setSystemItems] = useState<CheckItem[]>([
    ...
    { id: "fireDetection", topic: "מערכת גילוי אש- בדק ויזואלי לתקלות", isOk: null, isNotOk: null, comments: "" },
    { id: "sprinklers", topic: "ספרינקלרים – בדק ויזואלי לאיתור נזילות", isOk: null, isNotOk: null, comments: "" },
    { id: "fans", topic: "מפוחים – הפעלה חודשית", isOk: null, isNotOk: null, comments: "" },
    { id: "submersiblePumps", topic: "משאבות טבולות – הפעלה חודשית", isOk: null, isNotOk: null, comments: "" },
    { id: "electricGates", topic: "שערים חשמליים לחניון- בדק ויזואלי", isOk: null, isNotOk: null, comments: "" },
  ])

  const handleGeneralItemChange = (index: number, field: "isOk" | "isNotOk" | "comments", value: any) => {
  ...
  const handleSystemItemChange = (index: number, field: "isOk" | "isNotOk" | "comments", value: any) => {
  ...
  const generatePDF = (formData: any) => {
    ...
    const sanitizedAddress = formData.address.replace(/[^\w\s]/gi, "").replace(/\s+/g, "_")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(`אירעה שגיאה ביצירת ה-PDF: ${error instanceof Error ? error.message : "שגיאה לא ידועה"}`)
    } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden" dir="rtl">
      {/* טעינת סקריפטים של pdfmake */}
      <Script
      ...
      <div className="bg-[#091057] text-white p-4 md:p-6 text-center">
        ...
        <h1 className="text-xl md:text-3xl font-bold">ים ניהול ואחזקה – סיור ביקורת</h1>
      ...
      <form onSubmit={handleSubmit} className="p-4 md:p-6">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#024CAA] text-white">
                <th className="border border-[#DBD3D3] p-2 text-right">פרטי המנהל</th>
                <th className="border border-[#DBD3D3] p-2 text-right">פרטי הבניין ותאריך</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="border border-[#DBD3D3] p-4">
                  <div className="mb-2">
                    ...
                    <div className="p-2 bg-gray-50 rounded border border-gray-200">{managerName}</div>
                  </div>
                </td>
                <td className="border border-[#DBD3D3] p-4">
                  <div className="mb-2">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#024CAA] text-white">
                ...
                <th className="border border-[#DBD3D3] p-2 w-20 text-center">תקין</th>
                <th className="border border-[#DBD3D3] p-2 w-20 text-center">לא תקין</th>
                <th className="border border-[#DBD3D3] p-2 text-right">הערות</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={item.id} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  <td className="border border-[#DBD3D3] p-2 text-center">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleGeneralItemChange(index, "isOk", checked)}
                    />
                  </td>
                  <td className="border border-[#DBD3D3] p-2 text-center">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleGeneralItemChange(index, "isNotOk", checked)}
                    />
                  </td>
                  <td className="border border-[#DBD3D3] p-2">
                    <Textarea
                      id={`${item.id}-comments`}
                      value={item.comments}
                      onChange={(e) => handleGeneralItemChange(index, "comments", e.target.value)}
              ...
              <tr className="bg-gray-100">
                <td colSpan={4} className="border border-[#DBD3D3] p-2">
                  <Label htmlFor="manager-general-notes" className="block mb-1 font-bold">
                    הערות מנהל אחזקה:
                  </Label>
                  <Textarea
                    id="manager-general-notes"
                    value={managerNotes.general}
                    onChange={(e) => setManagerNotes({ ...managerNotes, general: e.target.value })}
        ...
        <div className="mb-8 overflow-x-auto">
          <h2 className="text-xl font-bold mb-4 text-[#091057]">בדיקת מערכות</h2>
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#024CAA] text-white">
                <th className="border border-[#DBD3D3] p-2 text-right">מערכת לבדיקה</th>
                <th className="border border-[#DBD3D3] p-2 w-20 text-center">תקין</th>
                <th className="border border-[#DBD3D3] p-2 w-20 text-center">לא תקין</th>
                <th className="border border-[#DBD3D3] p-2 text-right">הערות</th>
              </tr>
            ...
            <tbody>
                ...
                <tr key={item.id} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  <td className="border border-[#DBD3D3] p-2 text-center">
                    <Checkbox
                      id={`${item.id}-ok`}
                      checked={item.isOk === true}
                      onCheckedChange={(checked) => handleSystemItemChange(index, "isOk", checked)}
                    />
                  </td>
                  <td className="border border-[#DBD3D3] p-2 text-center">
                    <Checkbox
                      id={`${item.id}-notok`}
                      checked={item.isNotOk === true}
                      onCheckedChange={(checked) => handleSystemItemChange(index, "isNotOk", checked)}
                    />
                  </td>
                  <td className="border border-[#DBD3D3] p-2">
                    <Textarea
                      id={`${item.id}-comments`}
                      value={item.comments}
                      onChange={(e) => handleSystemItemChange(index, "comments", e.target.value)}
                      className="w-full min-h-[40px]"
                    />
                  </td>
                </tr>
              ...
              <tr className="bg-gray-100">
                <td colSpan={4} className="border border-[#DBD3D3] p-2">
                  <Label htmlFor="manager-systems-notes" className="block mb-1 font-bold">
                    הערות מנהל אחזקה:
                  </Label>
                  <Textarea
                    id="manager-systems-notes"
                    value={managerNotes.systems}
                    onChange={(e) => setManagerNotes({ ...managerNotes, systems: e.target.value })}
                    className="w-full min-h-[80px]"
                    placeholder="הערות נוספות של מנהל האחזקה..."
                  />
                </td>
        ...
        {submissionTime && (
          <div className="text-center mb-4 p-2 bg-gray-100 rounded-md">
            <p className="text-sm text-gray-600">זמן שליחת הטופס: {submissionTime}</p>
          </div>
        )}
        ...
        <div className="flex flex-col items-center mt-8">
          <Button
            ...
            disabled={isGenerating || !selectedClient || !pdfMakeLoaded || !vfsFontsLoaded}
          >
            {isGenerating ? (
              <>
          ...
          {!selectedClient && !isGenerating && (
            <p className="mt-2 text-red-500 text-sm">יש לבחור לקוח לפני הורדת ה-PDF</p>
          )}

          {(!pdfMakeLoaded || !vfsFontsLoaded) && (
            <p className="mt-2 text-amber-500 text-sm">טוען ספריות PDF... אנא המתן</p>
          )}

          {errorMessage && <div className="mt-4 p-4 rounded-md bg-red-100 text-red-800">{errorMessage}</div>}
        </div>
      </form>
    </div>
  )
